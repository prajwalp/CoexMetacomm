...
def iterator_x0(kernels):
    iterable = [(i,kernels) for i in range(R_array.size)]
    with Pool(25) as p:
...
if __name__ == '__main__':
    ...
    repNetworks = 50
    dataFolder =  "../data/revision_network_IID/ER_IID/"
    os.makedirs(dataFolder+"ER_iid_parallel_T-%d"%tmax,exist_ok=True)
    ...
    for repID in range(repNetworks):
        ...
        for i in range(S):
            f = farray[i]    
            er_kernel = mod_ss.find_effective_kernel(f, xi, er_network)
            kernels[i,:,:] = er_kernel

        coex_array, maxder = iterator_x0(kernels)
        np.savez_compressed(dataFolder+"ER_iid_parallel_T-%d/rep_%d.npz"%(tmax,repID), coex_array = coex_array, maxder = maxder, R_array = R_array, v2_array = v2_array)
