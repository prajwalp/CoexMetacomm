# Description: This script is used to generate the data for the BA network with IID noise. The script is parallelized over the different values of R and v2. The script generates the data for 25 different networks and saves the data in the folder "../data/revision_network_IID/BA_IID/BA_iid_parallel_T-%d/"%tmax. The data is saved in the form of a dictionary with keys "coex_array", "maxder", "R_array", "v2_array". The data is saved in the form of a compressed numpy file.
...
def loop_over_var(params):
    ...
    for j in range(len(v2_array)):
        ...
        del usol,rho_er_ms
    # print(i,time.time()-CUTSTART_T,"seconds")
...
if __name__ == '__main__':
    ...
    repNetworks = 25
    dataFolder =  "../data/revision_network_IID/BA_IID/"
    os.makedirs(dataFolder+"BA_iid_parallel_T-%d"%tmax,exist_ok=True)

    for repID in range(repNetworks):
        START_TIME = time.time()
        network = nx.barabasi_albert_graph(N,nn)
        ...
        for i in range(S):
            f = farray[i]    
            kernel = mod_ss.find_effective_kernel(f, xi, network)
            kernels[i,:,:] = kernel

        coex_array, maxder = iterator_x0(kernels,randomSeeds)
        
        np.savez_compressed(dataFolder+"BA_iid_parallel_T-%d/rep_%d.npz"%(tmax,25+repID), coex_array = coex_array, maxder = maxder, R_array = R_array, v2_array = v2_array)
